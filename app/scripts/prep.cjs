...
/* HDD_NO_FONTS is set by release.ps1 for artifacts meant to be published. The fonts in
 * app/fonts/ are the user's own licensed files, so a build that leaves this machine must
 * not embed them (ADR-011). Skipping them here means the packaged page holds no @font-face
 * and no font bytes at all; the CSS falls back to the system sans-serif. A local
 * `npm run dist`, without the variable, still embeds the font as before. */
const skipFonts = process.env.HDD_NO_FONTS === '1';

const fontFiles = skipFonts ? [] : fs.readdirSync(FONT_SRC)
...
if (faces.length === 0) {
  fontsCss += skipFonts
    ? '/* fonts deliberately excluded from this build (HDD_NO_FONTS=1) */\n'
    : '/* no font files found in app/fonts */\n';
} else {
...
if (skipFonts) {
  console.log('  fonts: excluded (HDD_NO_FONTS=1) — the UI will use the system sans-serif');
}
for (const fc of faces) {
